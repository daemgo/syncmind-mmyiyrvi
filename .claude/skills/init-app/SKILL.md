...
### Step 1.5: 学习项目现有设计风格（必须在生成代码之前执行）

在生成任何代码之前，**必须先读取以下文件**，理解项目已有的设计风格和组件用法：

1. `src/app/page.tsx` — 首页布局、卡片样式、间距节奏、渐变背景用法
2. `src/app/customers/page.tsx` — 列表页样式：Card hover 效果、Badge 状态色、Separator 分隔、backdrop-blur 顶栏
3. `src/app/globals.css` — 可用的颜色 token（primary、secondary、muted、accent、destructive）和主题变量

**强制要求**：
- 生成的代码必须**匹配或超过**这些文件的设计质量，不能低于现有页面的样式水平
- 必须使用 shadcn/ui 组件 + 语义化颜色 token（如 `text-muted-foreground`、`bg-primary`），不要用硬编码颜色
- 所有 Card 组件要有 `hover:shadow-md transition-shadow` 等交互反馈
- 页面顶部要有带 `backdrop-blur` 的标题栏
- 列表页使用 `grid gap-6 md:grid-cols-2 lg:grid-cols-3` 响应式布局
- 图表使用 `recharts`（项目已安装），不要用 div 占位
